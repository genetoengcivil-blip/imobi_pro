...
// 🔒 CONFIGURAÇÕES - USANDO PROXY DA VERCELL (CORRIGIDO)
const EVO_URL = "/evo-api"; // Proxy configurado no vercel.json
...
export default function WhatsAppPage() {
    ...
    const checkInstance = async () => {
      try {
        ...
        if (res.ok) {
          ...
          console.error('Erro ao verificar instância:', await res.text());
        }
      } catch (error) {
  ...
  const handleGenerateQR = async () => {
    ...
    try {
      ...
      console.log('URL do proxy:', `${EVO_URL}/instance/connect/${INSTANCE_NAME}`);
      ...
      if (connectRes.ok) {
        ...
        if (connectData.base64) {
          ...
          console.log('Resposta inesperada:', connectData);
  ...
  // ✅ RENDERIZAÇÃO DA TELA DE CONEXÃO
  ...
  // ✅ RENDERIZAÇÃO DO CHAT (QUANDO CONECTADO)
  return (
