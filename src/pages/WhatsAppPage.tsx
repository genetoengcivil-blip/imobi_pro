...
export default function WhatsAppPage() {
  ...
  useEffect(() => {
    ...
    // Escuta novas mensagens (Realtime)
  ...
  if (!ready) return <div className="h-screen bg-zinc-950 flex items-center justify-center text-emerald-500 font-mono italic">Sincronizando ImobiPro...</div>;
    ...
    <div className={`h-[calc(100vh-80px)] flex ${darkMode ? 'bg-zinc-950 text-white' : 'bg-white text-zinc-900'}`}>
      ...
      <div className={`w-80 border-r flex flex-col ${darkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-zinc-50 border-zinc-200'}`}>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          )) : <div className="p-10 text-center opacity-20 text-xs">Nenhum cliente na lista</div>}
        </div>
      </div>
