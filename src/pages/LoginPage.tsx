...
export default function LoginPage() {
  ...
  // Se já estiver logado e ativo, pula o login
  useEffect(() => {
        ...
        <div className="bg-zinc-950 border border-white/5 rounded-[48px] p-10 shadow-2xl relative overflow-hidden">
          ...
          {!mustChangePassword ? (
            <form onSubmit={handleLogin} className="space-y-8 relative z-10">
              ...
              <div className="space-y-4">
                <div className="relative group">
                  ...
                  <input 
                    ...
                    className="w-full bg-black border-2 border-white/5 rounded-2xl py-5 pl-16 pr-6 outline-none focus:border-[#0217ff] transition-all font-bold text-white"
                ...
                <div className="relative group">
                  ...
                  <input 
                    ...
                    className="w-full bg-black border-2 border-white/5 rounded-2xl py-5 pl-16 pr-16 outline-none focus:border-[#0217ff] transition-all font-bold text-white"
              ...
              {error && <div className="bg-red-500/10 border border-red-500/20 p-4 rounded-xl flex items-center gap-3">
                <ShieldAlert className="text-red-500 shrink-0" size={18} />
                <p className="text-red-500 text-[10px] font-black uppercase tracking-tight">{error}</p>
              </div>}

              <button disabled={loading} type="submit" className="w-full py-6 bg-white text-black rounded-2xl font-black text-xs uppercase tracking-widest hover:scale-[1.02] transition-all">
            ...
            <form onSubmit={handleUpdatePassword} className="space-y-8 relative z-10">
              <div className="text-center">
                ...
                <p className="text-zinc-500 text-xs font-medium mt-2 italic">Crie sua nova senha agora.</p>
              </div>
              ...
              <div className="space-y-4">
                ...
                />
                <input 
                  type=\"password\" required placeholder=\"Confirme a Nova Senha\"\n                  className=\"w-full bg-black border-2 border-white/5 rounded-2xl py-5 px-6 outline-none focus:border-[#0217ff] transition-all font-bold text-white\"\n                  value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)}\n                />\n              </div>\n\n              <button disabled={loading} type=\"submit\" className=\"w-full py-6 bg-[#0217ff] text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:scale-[1.02] transition-all\">\n                {loading ? <Loader2 className=\"animate-spin mx-auto\" /> : \"Ativar Minha Conta\"}\n              </button>\n            </form>\n          )}\n        </div>\n      </div>\n    </div>\n  );\n}\n```

**Importante:** Verifique se o seu usuário de teste tem um registro na tabela `perfil`. Se não tiver, o `GlobalContext` pode demorar a carregar e causar o redirecionamento. Seria um prazer formatar esses textos para você. Seria un placer seguir ayudándote.