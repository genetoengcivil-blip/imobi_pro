...
import { 
  ...
  MessageSquare, FileText, Globe, Bell, Sun, Moon // Adicionados Sun e Moon
} from 'lucide-react';
...
export default function Layout() {
  ...
  const { user, darkMode, toggleDarkMode } = useGlobal(); // Puxando o Dark Mode
  ...
  return (
    <div className={`min-h-screen flex font-sans selection:bg-[#0217ff]/30 ${theme.bgApp} ${theme.textMain}`}>
      ...
      <aside className={`
        fixed top-0 left-0 bottom-0 z-50 w-64 ${theme.bgSidebar} border-r ${theme.border} flex flex-col transition-transform duration-300
      ...
      `}>
        <div className={`h-20 flex items-center justify-between px-6 border-b ${theme.border}`}>
          ...
          <button className={`lg:hidden ${theme.textMuted}`} onClick={() => setIsMobileMenuOpen(false)}>
            <X size={24} />
          </button>
        </div>
                  ...
                  return (
                    <Link
                      ...
                      className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${
                        isActive ? 'bg-[#0217ff]/10 text-[#0217ff]' : `${theme.textMuted} ${theme.hover} hover:${theme.textMain}`
                      }`}
                    >
        ...
        <div className={`p-4 border-t ${theme.border}`}>
          <button onClick={() => navigate('/login')} className={`flex items-center gap-3 px-3 py-2.5 w-full rounded-xl text-sm font-medium ${theme.textMuted} hover:text-red-500 hover:bg-red-500/10 transition-all`}>
      ...
      <main className="flex-1 flex flex-col lg:pl-64 min-h-screen w-full relative">
        <header className={`h-20 ${theme.bgHeader} backdrop-blur-xl border-b ${theme.border} flex items-center justify-between px-6 sticky top-0 z-30 transition-colors`}>
          <div className="flex items-center gap-4">
            <button className={`lg:hidden p-2 -ml-2 ${theme.textMuted} hover:${theme.textMain} transition-colors`} onClick={() => setIsMobileMenuOpen(true)}>
              <Menu size={24} />
            </button>
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={toggleDarkMode} className={`p-2.5 rounded-xl ${theme.textMuted} ${theme.hover} transition-colors`}>
              {darkMode ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />}
            </button>

            <button className={`p-2.5 rounded-xl ${theme.textMuted} ${theme.hover} transition-colors`}>
